...
# nonsense value, just a placholder
index = 100
fps = 60
...
instruments = 6
boxes = []
# a list of lists containing inactive beats/rectangles
clicked = [[-1 for _ in range(beats)] for _ in range(instruments)]
# active channels/instruments; init with six 1s
...
# some instruments occupy more than one channel
pygame.mixer.set_num_channels(instruments * 3)


def play_notes():
    for i in range(len(clicked)):
        # if a beat is selected...
...
while run:
    ...
    if beat_changed:
        play_notes()
        beat_changed = False
    # processing keyboard events, etc.
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            run = False
        if event.type == pygame.MOUSEBUTTONDOWN and not save_menu and not load_menu:
            for i in range(len(boxes)):
                # check if the mouse position colides with the given box
                if boxes[i][0].collidepoint(event.pos):
                    coords = boxes[i][1]
                    # activating the beat/rectangle
        ...
        if event.type == pygame.MOUSEBUTTONUP and not save_menu and not load_menu:
            if play_pause.collidepoint(event.pos):
                ...
                beats += 1
                for i in range(len(clicked)):
                    # nice little trick to add a new object
                    clicked[i].append(-1)
    ...
    # closely associated with set FPS. FPS * 60
